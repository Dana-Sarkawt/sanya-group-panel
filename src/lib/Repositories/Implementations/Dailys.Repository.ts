...
export class DailysRepository implements IDailysRepository {
	...
	): Promise<PostgrestSingleResponse<Database['public']['Tables']['Dailys']['Row']>> {
		try {
			const response = await Supabase.client.from('Dailys').insert(request).select('*').single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	async readDailyAsync(
		...
		try {
			const response = await Supabase.client.from('Dailys').select('*').eq('id', id).single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	): Promise<PostgrestSingleResponse<Array<Database['public']['Tables']['Dailys']['Row']>>> {
		try {
	...
	async readDailysTotalPriceAsync(projectId: number): Promise<number> {
		try {
			...
			const totalPrice = response.data.reduce((acc, curr) => acc + curr.price, 0);
			return totalPrice;
		} catch (error) {
			throw error;
		}
