...
export class DepositsRepository implements IDepositsRepository {
	async createDepositAsync(
		...
		try {
			const response = await Supabase.client.from('Deposits').insert(request).select('*').single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	async readDepositAsync(
		...
		try {
			const response = await Supabase.client.from('Deposits').select('*').eq('id', id).single();
			return response;
		} catch (error) {
			throw error;
		}
