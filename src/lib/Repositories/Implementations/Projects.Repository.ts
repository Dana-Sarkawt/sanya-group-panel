...
export class ProjectsRepository implements IProjectsRepository {
	...
	): Promise<PostgrestSingleResponse<Database['public']['Tables']['Projects']['Row']>> {
		try {
			const response = await Supabase.client.from('Projects').insert(request).select('*').single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	async readProjectAsync(
		...
		try {
			const response = await Supabase.client.from('Projects').select('*').eq('id', id).single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	): Promise<PostgrestSingleResponse<Array<Database['public']['Tables']['Projects']['Row']>>> {
		try {
