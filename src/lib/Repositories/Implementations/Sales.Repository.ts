...
export class SalesRepository implements ISalesRepository {
	...
	): Promise<PostgrestSingleResponse<Database['public']['Tables']['Sales']['Row']>> {
		try {
			const response = await Supabase.client.from('Sales').insert(request).select('*').single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	async readSaleAsync(
		...
		try {
			const response = await Supabase.client.from('Sales').select('*').eq('id', id).single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	): Promise<PostgrestSingleResponse<Array<Database['public']['Tables']['Sales']['Row']>>> {
		try {
	...
	async readSalesWithoutFilterAsync(
		projectId: number
	): Promise<PostgrestSingleResponse<Array<{ Id: number; Description: string | null }>>> {
		try {
