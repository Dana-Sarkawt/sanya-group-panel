...
export class WorkersRepository implements IWorkersRepository {
	...
	): Promise<PostgrestSingleResponse<Database['public']['Tables']['Workers']['Row']>> {
		try {
			const response = await Supabase.client.from('Workers').insert(request).select('*').single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	async readWorkerAsync(
		...
		try {
			const response = await Supabase.client.from('Workers').select('*').eq('id', id).single();
			return response;
		} catch (error) {
			throw error;
		}
	...
	): Promise<PostgrestSingleResponse<Array<Database['public']['Tables']['Workers']['Row']>>> {
		try {
	...
	> {
		try {
			const response = await Supabase.client.rpc('count_financials_by_workers', {
				worker_ids: ids
			});
			if (response.error) throw new Error(response.error.message);
			return response;
