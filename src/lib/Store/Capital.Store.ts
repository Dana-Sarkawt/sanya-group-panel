...
const createCapitalStore = () => {
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Capitals']['Row']>) => set(data),
		create: async (data: Database['public']['Tables']['Capitals']['Insert']) => {
			try {
		...
		getAllWithoutFilter: async (projectId: number) => {
			try {
				const response = await capitalsRepository.readCapitalsWithoutFilterAsync(projectId);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-capitals'));
				}
				return {
		...
		getTotalPrice: async (projectId: number) => {
			try {
				const response = await capitalsRepository.readCapitalsTotalPriceAsync(projectId);
				if (response == 0) {
		...
		update: async (data: Database['public']['Tables']['Capitals']['Update']) => {
			try {
