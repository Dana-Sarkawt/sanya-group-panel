...
const createDailyStore = () => {
	const { subscribe, set, update } = writable<Store<Database['public']['Tables']['Dailys']['Row']>>(
		{
			data: [],
			count: 0
		}
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Dailys']['Row']>) => set(data),
		...
		getAllWithoutFilter: async (projectId: number) => {
			try {
				const response = await dailysRepository.readDailysWithoutFilterAsync(projectId);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-daily'));
				}
				return {
		...
		getTotalPrice: async (projectId: number) => {
			try {
				const response = await dailysRepository.readDailysTotalPriceAsync(projectId);
