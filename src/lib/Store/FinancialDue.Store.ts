...
const createFinancialDueStore = () => {
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Financial Dues']['Row']>) => set(data),
		create: async (data: Database['public']['Tables']['Financial Dues']['Insert']) => {
			try {
				const response = await financialDuesRepository.createFinancialDueAsync(data);
				if (response.error) {
		...
		get: async (id: number) => {
			try {
				const response = await financialDuesRepository.readFinancialDueAsync(id);
				if (response.error) {
		...
		update: async (data: Database['public']['Tables']['Financial Dues']['Update']) => {
			try {
				const response = await financialDuesRepository.updateFinancialDueAsync(data);
				if (response.error) {
		...
		delete: async (id: number) => {
			try {
				const response = await financialDuesRepository.deleteFinancialDueAsync(id);
				if (response.error) {
					throw new Error(get(_)('failed-to-delete-financial-due'));
				}
				toastStore.success(get(_)('financial-due-deleted-successfully'));
				update((store) => {
					store.data = store.data.filter((financialDue) => financialDue.id !== id);
					store.count--;
					return store;
				});
				return response;
