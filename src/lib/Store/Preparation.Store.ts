...
const createPreparationStore = () => {
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Preparations']['Row']>) => set(data),
		create: async (data: Database['public']['Tables']['Preparations']['Insert']) => {
			try {
				const response = await preparationsRepository.createPreparationAsync(data);
		...
		getAll: async (options?: GenericListOptions) => {
			try {
				const response = await preparationsRepository.readPreparationsAsync(options);
		...
		getAllWithoutFilter: async (projectId: number) => {
			try {
				const response = await preparationsRepository.readPreparationWithoutFilterAsync(projectId);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-preparation'));
				}
				return {
		...
		getOverhaulDepositsByProjectId: async (projectId: number) => {
			try {
				const response =
					await preparationsRepository.readOverhaulDepositsByProjectIdAsync(projectId);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-preparation'));
				}
		...
		getOverhaulFinancialsByProjectId: async (projectId: number) => {
			try {
				const response =
					await preparationsRepository.readOverhaulFinancialsByProjectIdAsync(projectId);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-preparation'));
				}
				return response.data;
		...
		getDepositsByPreparationIds: async (ids: number[]) => {
			try {
				const response = await preparationsRepository.readDepositsByPreparationIdsAsync(ids);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-preparation'));
				}
				return response.data;
		...
		getFinancialByPreparationIds: async (ids: number[]) => {
			try {
				const response = await preparationsRepository.readFinancialByPreparationIdsAsync(ids);
				if (response.error) {
					throw new Error(get(_)('failed-to-get-preparation'));
				}
				return response.data;
		...
		update: async (data: Database['public']['Tables']['Preparations']['Update']) => {
			try {
				if (!data.id) {
					throw new Error(get(_)('id-is-required'));
				}

				const response = await preparationsRepository.updatePreparationAsync(data);
		...
		delete: async (id: number) => {
			try {
				const response = await preparationsRepository.deletePreparationAsync(id);
