...
const createProjectStore = () => {
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Projects']['Row']>) => set(data),
		create: async (data: Database['public']['Tables']['Projects']['Insert']) => {
			try {
		...
		getTotalPrice: async () => {
			try {
				const response = await projectsRepository.readTotalPriceOfProjectsAsync();
				return response;
			} catch (error) {
				if (error instanceof Error) {
					toastStore.error(error.message);
		...
		update: async (data: Database['public']['Tables']['Projects']['Update']) => {
			try {
				if (!data.id) {
				...
				update((store) => {
					const index = store.data.findIndex((project) => project.id === data.id);
