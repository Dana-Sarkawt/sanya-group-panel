...
const createWorkerStore = () => {
	...
	return {
		subscribe,
		set: async (data: Store<Database['public']['Tables']['Workers']['Row']>) => set(data),
		...
		getAllWithoutFilter: async (projectId: number) => {
			try {
				const response = await workersRepository.readWorkersWithoutFilterAsync(projectId);
				if (response.error) {
					throw new Error(response.error.message);
				}
				return {
		...
		getOverhaulDepositsByProjectId: async (projectId: number) => {
			try {
				const response = await workersRepository.readOverhaulDepositsByProjectIdAsync(projectId);
				if (response.error) {
					throw new Error(response.error.message);
				}
		...
		getOverhaulFinancialsByProjectId: async (projectId: number) => {
			try {
				const response = await workersRepository.readOverhaulFinancialsByProjectIdAsync(projectId);
				if (response.error) {
					throw new Error(response.error.message);
				}
				return response.data;
